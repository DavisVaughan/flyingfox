...
1)  Create a free account on [Quandl](https://www.quandl.com/) and find
    your API Key in Account Settings. 2) Add the API key as the R
    environment variable, `QUANDL_API_KEY` (access your `.Renviron`
    variable with `usethis::edit_r_environ()`).
2)  “Ingest” the Quandl data with `flyingfox::fly_ingest()`.
